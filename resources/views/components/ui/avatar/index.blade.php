...
@php
	...
	$avatarColor = match ($color) {
		...
		default => 'bg-neutral-200 [&>[data-slot=icon]]:text-white!',
	};
	...
	$badgeClasses = [
		'absolute ring-[2px] ring-white z-10',
		'flex items-center justify-center tabular-nums overflow-hidden',
		'text-[.625rem] text-black font-medium',
		'after:absolute after:inset-[3px] after:bg-white ' => $badgeVariant === 'outline',
...
<div class="relative w-fit" data-slot="avatar" data-size="{{ $avatarSize }}">
	...
	>
		...
		@if ($badge instanceof \Illuminate\View\ComponentSlot)
			<div
				{{ $badge->attributes->class(Arr::toCssClasses($badgeClasses)) }}
				aria-hidden="true"
				>{{ $badge }}
			</div>
		@elseif ($badge)
			<div
				class="{{ Arr::toCssClasses($badgeClasses) }}"
				aria-hidden="true"
			>
