...
<label
	{{ $attributes->whereDoesntStartWith(['wire:model', 'x-model'])->class(Arr::toCssClasses($classes)) }}
	role="{{ $role }}"
...
>
	...
	@if ($checkboxVariant)
		<div class="relative inline-grid place-items-center size-5 mr-2">
			<span
				class="size-4.5 rounded ring-1 ring-neutral-100 transition shadow"
				x-bind:class="{
			...
			<x-ui.icon
				name="check"
				class="absolute size-4 text-(--color-primary-fg)! transition-all duration-200"
				x-bind:class="{
	...
	@else
		<x-ui.icon
			name="check"
			variant="mini"
			class="{{ Arr::toCssClasses([...$iconClasses, 'opacity-0 peer-checked:opacity-100 transition-opacity duration-150']) }}"
			aria-hidden="true"
		/>
	@endif
	@if (filled($icon))
	...
	@if ($slot->isNotEmpty())
		<span
			class="col-start-3 whitespace-nowrap flex items-center justify-between gap-4"
		>
			<span>{{ $slot }}</span>
