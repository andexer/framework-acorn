...
@php
	...
	$variantClasses = match ($variant) {
		'soft' => 'text-neutral-800 hover:bg-neutral-100 focus-within:bg-neutral-100 ',
		'danger' => 'text-red-600 hover:bg-red-50 focus-within:text-red-600 focus-within:bg-red-50 ',
		default => '',
	};
	...
	$iconClasses = [
		...
		match ($variant) {
			'soft' => '',
			'danger' => 'text-red-500! ! group-focus-within:text-red-500! !',
			default => '',
		},
	];
	...
	$classes = [
		...
		'data-active:bg-neutral-950/[3%] %]',
		$variantClasses . ' cursor-pointer' => !$disabled,
		'opacity-50 cursor-not-allowed text-neutral-500 ' => $disabled,
	];
...
@if ($isDefaultDropdownVariant)
	<x-ui.button.abstract
		:$as
		:attributes="$attributes->class(Arr::toCssClasses($classes))->merge([
		'disabled' => $disabled,
	...
	>
		...
		<span
			class="col-start-2 whitespace-nowrap flex items-center justify-between gap-4_"
		>
...
@elseif ($readOnly)
	<div
		class="text-center col-span-full text-neutral-800 w-full px-3 py-1.5 text-sm"
		{{ $attributes }}
	>
		{{ $slot }}
	</div>
