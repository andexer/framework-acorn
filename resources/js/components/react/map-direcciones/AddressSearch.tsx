...
interface AddressSearchProps {
	onSelect: (lat: number, lng: number, isVenezuela: boolean) => void;
}
					...
					<ul className="py-1">
						{results.map((result) => (
							<li
								key={result.place_id}
								onClick={() => {
									const isVenezuela = (result as any).address?.country_code === 've';
									onSelect(Number(result.lat), Number(result.lon), isVenezuela);
